...
- [Spraay Direct](https://github.com/plagtech/spraay-direct-proxy) — Instant USDC payouts for GPU operators. First x402 service for the operator payout side: GPU hosts running vLLM, Ollama, or ComfyUI connect to the Spraay gateway and receive USDC on Base the moment each inference job completes ($0.03/job). Batch split payouts to multiple wallets via on-chain batch contract. No 2-week PayPal wait, no platform fees. [Gateway](https://gateway.spraay.app/api/v1/gpu-direct/operators) | [Operator Proxy](https://github.com/plagtech/spraay-direct-proxy)
