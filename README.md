...
- [byte-mcp-server](https://github.com/0rkz/byte-mcp-server) - Per-byte USDC data feeds and oracles for AI agents on Arbitrum. 14 MCP tools across discovery, subscribe, and `byte_buy_data` (x402 one-off via the gateway at https://x402.payperbyte.io/feeds). 15+ first-party publishers across markets, weather, threat-intel, legal, scientific, and fact-oracles. Every settlement emits an EIP-712 PayloadAttestation; subscribers re-derive keccak256 over received bytes to verify against the on-chain hash — fails closed on tamper. $0.001–$0.069 USDC per call. ([npm](https://www.npmjs.com/package/byte-mcp-server)) ([Smithery](https://mcp.smithery.run/byte-library)) ([MCP Registry](https://registry.modelcontextprotocol.io/v0/servers?search=byte))
