...
- [Spraay MCP Server](https://smithery.ai/server/@plagtech/spraay-x402-mcp) - MCP server for multi-chain batch payments, payroll, token swaps, bridge, escrow, AI inference (43+ models), Robot Task Protocol (RTP), agent wallets, supply chain (SCTP), compute futures, compute (LLM/image/video/TTS/STT), Solana DeFi (Jupiter/Helius/Pyth), research & reference (23 academic/chemistry/biomedical endpoints), search/RAG, and more. Backed by 170 x402 gateway endpoints across 37 categories and 16 chains (Base, Ethereum, Solana, Bitcoin, Arbitrum, Polygon, BNB, Avalanche, Unichain, Plasma, BOB, Bittensor, XRP, Stacks, Stellar, Canton). $0.001-$0.50 USDC. ([GitHub](https://github.com/plagtech)) ([Gateway](https://gateway.spraay.app)) ([Docs](https://docs.spraay.app))
...
- [Spraay Gateway](https://gateway.spraay.app) - Multi-chain batch payment protocol and x402 gateway (v3.8.1). **170 endpoints** (145 paid + 25 free) across **37 categories** and **16 chains** (Base, Ethereum, Arbitrum, Polygon, BNB, Avalanche, Unichain, Plasma, BOB, Solana, Bittensor, Bitcoin, XRP Ledger, Stacks, Stellar, Canton). Primary differentiator: **batch payments** — pay N recipients in one transaction on any supported chain. Categories span Batch Payments, Payroll, Token Swaps, Bridge & Cross-chain, Escrow, AI Inference (43+ models via Bittensor SN64), Robot Task Protocol (RTP — agents hire physical robots via x402 micropayments), Agent Wallets (factory + session keys on Base), Supply Chain (SCTP — 8 endpoints for PO/invoice/supplier workflows), Compute Futures (prepaid credits with tier discounts up to 15%), Research & Reference (23 endpoints — academic papers, preprints, chemistry, biomedical, demographics, dictionary), Compute (LLM, image, video, TTS, STT, embeddings), Solana DeFi (Jupiter, Helius DAS, Pyth), Search & RAG, GPU, Oracle, Portfolio, Contract read/write, DeFi positions, XRP & Stellar batch payments, and more. Verified batch contract on Base: [`0x1646...5eEC`](https://basescan.org/address/0x1646452F98E36A3c9Cfc3eDD8868221E207B5eEC). Compute Futures escrow: [`0xaf4e...9118`](https://basescan.org/address/0xaf4e43d512c73c673176962dca6b73f0fcff9118). Open specs: [BPA 1.0](https://docs.spraay.app/bpa/1.0/) (Batch Payment Abstraction). CDP Bazaar indexed (20+ endpoints). $0.001–$0.50 USDC per call. Also available as an [MCP server](https://smithery.ai/server/@plagtech/spraay-x402-mcp). ([Docs](https://docs.spraay.app)) ([GitHub](https://github.com/plagtech)) ([Discovery](https://gateway.spraay.app/.well-known/x402))
